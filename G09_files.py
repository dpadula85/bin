...
class input_file:
  ...
  def get_basis(self):
    ...
    if self.keywords:
      for keyword in self.keywords.split():
        for basis_set in map(str.lower, list(itertools.chain.from_iterable(dft.basis.values()))):
          if SequenceMatcher(None, keyword.lower(), basis_set).ratio() > 0.65:
            basis = keyword
            stop = True
            break
        if stop:
  ...
  def write_job(self, opts_dict=None):
    ...
    f = open('%s' % opts_dict['name'], 'w')
      
    if opts_dict['nproc']: f.write('%%nproc=%d\n' % opts_dict['nproc'])
    if opts_dict['mem']: f.write('%%mem=%d%s\n' % (opts_dict['mem'], opts_dict['mem_unit']))
    if opts_dict['chk']: f.write('%%chk=%s.chk\n' % opts_dict['chk'])
...
# It is easy, but now I feel lazy
  ...
  def write_stick(self):
    ...
    f = open('%s_stick.txt' % self.file.split('.')[0], 'w')
