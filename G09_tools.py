...
def options():
    ...
    #The argument is mandatory, but some following options (e.g. --coup) do not
    #require it
    ...
    #Extraction of Excited States properties and creation of a stick spectra file
    parser.add_argument('--stick', help='Extracts Excited States properties and writes it to a stick file.', action='store_true')
    ...
    for coupling calculations with G09. This option does not require a file to be specified.''', choices=['all', 'calclist'])

    #Preparation of EXAT calculations including EXAT calculations for each couple
    #of chromophores constituting the system, or not.
    #bb is for a backbone calculation, including only n->pi* and pi->pi* transitions
    #described by states 1 and 4 of NMA.
    parser.add_argument('--exat', help='''Prepares files needed for EXAT calculations.
    This option does not require a file to be specified.''', choices=['couples', 'system', 'bb'])
...
if __name__ == '__main__':
    ...
    if args.coup:
        ...
        dirs = sorted(filter(lambda x: re.match('\d+', x), os.listdir(os.getcwd())))
        ...
        if args.coup == 'all':
            ...
            for chrom1 in dirs[:]:
                for chrom2 in filter(lambda x: x != chrom1, dirs):
                    ...
                    # Any change in the calculation method
                    # should be done in G09_files module
        ...
        if args.exat == "couples":
            ...
            for chrom1 in dirs[:]:
                for chrom2 in filter(lambda x: x != chrom1, dirs):
                    ...
                    #Create a directory called exat_chrom1_chrom2 and copy there
                    ...
                    shutil.copyfile(os.path.join(chrom1, '%s.log' % chrom1), os.path.join(exat_dir, chrom1, '%s.log' % chrom1))
                    shutil.copyfile(os.path.join(chrom2, '%s.log' % chrom2), os.path.join(exat_dir, chrom2, '%s.log' % chrom2))
                    shutil.copyfile(os.path.join(coup_dir, '%s.log' % coup_dir), os.path.join(exat_dir, coup_dir, '%s.log' % coup_dir))

                    #Write chromlist.in in the exat_dir for the partial job
                    chromlist_part = open(os.path.join(exat_dir, 'chromlist.in'), 'w')
