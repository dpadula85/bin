...
def options():
    ...
    inp.add_argument('--dist', type=str, dest='DistFile',
                     help='''Distribution along MD trajectory.''')
...
def plot(qmdata, mmdata, dist, **kwargs):
    ...
    gs = gridspec.GridSpec(2, 1, hspace=0.15, height_ratios=[ 0.5, 1])
    ...
    # Symmetrise
    # qmdata_sym = qmdata.copy()
    # qmdata_sym[:,0] *= -1
    # qmdata = np.r_[ qmdata, qmdata_sym ]

    mmdata_sym = mmdata.copy()
    mmdata_sym[:,0] *= -1
    mmdata = np.r_[ mmdata, mmdata_sym ]
    ...
    # Plot Distribution
    ax = plt.subplot(gs[0])
    nbins = int(np.log2(dist.shape[0]) + 1)
    n, bins, patches = ax.hist(dist, bins=nbins, histtype='bar', rwidth=0.75,
...
if __name__ == '__main__':
    ...
    dist = np.loadtxt(Opts["DistFile"])[:,:2][:,1]
    plot(qmdata, mmdata, dist, **Opts)
