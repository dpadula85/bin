...
def options():
    ...
    parser.add_argument('-w', '--wrap', default=False, action="store_true",
    help='''Number of repetitions of the Tubular unit cell.''')
...
if __name__ == '__main__':
    ...
    #
    range1 = np.arange(-30,30)
    range2 = np.arange(-30,30)
    ...
    sheet = np.c_[np.ones(len(coords)), coords, np.zeros(len(coords))]

    #
    # Save unwrapped 60x60 grid
    #
    ...
    if args.wrap:
        ...
        #
        with open(args.output + ".tubecell.xyz", "w") as f:
            f.write("%d\n\n" % len(sheet))
            np.savetxt(f, sheet, fmt="%5d %14.8f %14.8f %14.8f")
